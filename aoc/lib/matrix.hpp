 ...
 * Time: O(N^3)
 */
  template <typename enable_if<ROWS == COLS && is_convertible_v<T, double>, bool>::type = true>
 ...
 * Time: O(N^3)
 */
  template<typename enable_if<ROWS == COLS, bool>::type = true, typename enable_if<is_convertible_v<T, double>, bool>::type = true>
  constexpr pair<Matrix<double, ROWS, COLS>, int> inverse() const {
	...
	for (int i = 0; i < N; ++i) {
		...
		if (fabs(A[r][c]) < _MATRIX_EPS) return {A, i};
